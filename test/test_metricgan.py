"""MetricGAN+ test. Kullanıcıdan dosya seçtirir ya da sentetik sinyal üretir."""
...
print("Model yükleniyor (ilk çağrıda HuggingFace'ten indirir)...")
model = MetricGANDenoiser()
model.load()

print("Temizleniyor...")
denoised = model.process(noisy)

save_audio("noisy_input_mgan.wav", noisy)
save_audio("denoised_mgan.wav", denoised)

print(f"Gürültülü RMS : {np.sqrt(np.mean(noisy ** 2)):.4f}")
print(f"Temizlenmiş RMS: {np.sqrt(np.mean(denoised ** 2)):.4f}")
print("WAV'lar yazıldı: noisy_input_mgan.wav, denoised_mgan.wav")
