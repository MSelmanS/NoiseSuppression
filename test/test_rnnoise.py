"""RNNoise test. Kullanıcıdan dosya seçtirir ya da sentetik sinyal üretir."""
...
model = RNNoiseDenoiser()
model.load()

print("Temizleniyor...")
denoised = model.process(noisy)

save_audio("noisy_input_rnn.wav", noisy)
save_audio("denoised_rnn.wav", denoised)

print(f"Gürültülü RMS : {np.sqrt(np.mean(noisy ** 2)):.4f}")
print(f"Temizlenmiş RMS: {np.sqrt(np.mean(denoised ** 2)):.4f}")
print("WAV'lar yazıldı: noisy_input_rnn.wav, denoised_rnn.wav")
